...
#     # Convert the free_slots to a JSON response
...
def langcahin_output_parse(transcript):
    ...
    Agreed: Have the user agreed and booked an apointment?\
    Have Ai agent booked the apointment successfully?\
    Answer True if yes,\
    False if not.

    Start datetime: Extract start datetime\
    from the given transcript\
    if apointment is successfully booked otherwise answer None\

    end datetime: Extract end datetime\
    from the given transcript\
    if apointment is successfully booked otherwise answer None\
    ...
    start datetime and end datetime, both are compulsory
    text: {text}
    """
        
    prompt_template = ChatPromptTemplate.from_template(review_template)

    agreed_schema = ResponseSchema(name="Agreed",
    description="Did the user agreed and booked an apointment?\
    Did Ai agent booked the apointment successfully?\
    Answer True if yes,\
    False if not.")

    start_date_schema = ResponseSchema(name="start datetime",
    description="Extract start datetime\
    from the given transcript\
    if apointment is successfully booked otherwise answer None")

    end_date_schema = ResponseSchema(name="end datetime",
    description="Extract end datetime\
    from the given transcript\
    if apointment is successfully booked otherwise answer None")
    ...
    Agreed: Did the user agreed and booked an apointment?\
    Did Ai agent booked the apointment successfully?\
    Answer "True" if yes,\
    "False" if not.
    ...
    from the given transcript\
    if apointment is successfully booked answer in the format of \
    start datetime: 09 August 2024, 09:00AM (these are dummy date and time, dont use this)\
    end datetime: 09 August 2024, 10:00AM (these are dummy date and time, dont use this)\
...
# langcahin_output_parse("""AI: Hello? This is Mary from doctor ai assistant. How can I assist you today?
